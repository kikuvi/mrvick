namespace Shopfront.API.DTOs;

public record UserDto(string Id, string Email, string FullName, string? PhoneNumber, bool EmailConfirmed, bool MustChangePassword, bool IsActive);

public record CreateUserDto(string Email, string FullName, string? PhoneNumber);

public record UpdatePasswordDto(string NewPassword);

public record UpdateUserDto(string FullName, string? PhoneNumber, string Email);

public record ChangeOwnPasswordDto(string CurrentPassword, string NewPassword);
